...
### 迭代 6c（2026-08-01）— 登录态不一致修复（用户确认场景）

**用户报告**: 未登录却进入主页新建任务页，点发送无反应，日志报"未登录"。

**根因**: 渲染层登录态（localStorage token）与主进程会话（session.json）不一致——
localStorage 残留 token（mock 时代/早期登录）放行路由守卫进入 /home，
主进程 session 为空 → conversation:create 拒绝"未登录" → 发送静默失败。

**修复**:
1. 新增 `session:check` IPC（主进程会话为权威登录态）
2. 路由守卫改为异步校验主进程会话：未登录 → 清除本地残留 token → 回登录页
